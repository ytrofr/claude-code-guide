...
**Evidence**: Apr 2026 — `block-root-file-creation.sh` had hardcoded `PROJECT_ROOT="$HOME/LimorAI-Knowledge"` (a stale path from a renamed branch). The hook silently failed to protect any project. Fix: `${CLAUDE_PROJECT_DIR:-$PWD}`. Verified: now blocks root file creation in any project directory.
